...
// interfaces (DTOs / response objects), so the schema walker can resolve a typed
// Express handler's request/response body structure. It indexes declared shape
// only — property name + declared type — over the tree-sitter-typescript AST.
//
// This mirrors nestjs/typeindex.go: both TS providers need the same declared-type
// index, but the two providers are decoupled (a repo is one or the other), so the
// index is replicated here rather than imported across sibling providers. The
// natural consolidation is to promote it into lang/tsjs (as lang/kotlin already
// hosts kotlin.IndexTypes) and have both providers delegate — a follow-up, kept
// out of this change to leave the benchmarked NestJS provider untouched.
...
// classDef builds a TypeDef from a class_declaration: its field definitions and
// the first `extends` supertype (for inherited fields).
