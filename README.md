...
internal/model         the JSON output contract + deterministic identity/sort
...
`uncertain` nodes): DB detection (JPA/JDBC), OpenAPI ingestion, gRPC, full K8s
topology, running `helm`/`kustomize` (traced statically, never rendered),
Spring Cloud Config Server / secret managers, version history.
