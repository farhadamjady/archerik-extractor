...
pattern recurs across four services, `high`). Still open (not yet logged): Micronaut RxJava
`Maybe<T>`/`Single<T>` reactive wrappers not unwrapped (policy-search-service `Maybe<FindPolicyQueryResult>`).
No fixes applied this pass (user deferred); extractor tree unchanged (only bench artifacts
regenerated). Full gate green.
